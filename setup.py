...
def set_sudo_pass():
    ...
    for attempt in range(3): # three attempts given 
        ...
        pwd = utils.gtpass("root" , "System(Ubuntu)")
        ...
        if res.returncode == 0 : 
            ...
            const.UBUNTU_ROOT_PWD = pwd 
            return const.SUCCESS # password updated
        # wrong password 
...
def run_as_sudo(command : list[str]):  # setting hint for command argument
    '''
    Run Commands As Root User For System Level Operations  And Returns The Output Of it Failed if cannot execute
    '''
    # getting root password 
      
    # Ensuring the password is set
    if const.UBUNTU_ROOT_PWD is '' :
        if set_sudo_pass() == const.FAILED:
            return const.FAILED
    # executing commands with root access
    result  = subprocess.run(['sudo' ,'-S'] + command ,  # adding sudo to the command 
                        input=const.UBUNTU_ROOT_PWD + '\n', # sending password 
                        capture_output=True,
                        text=True)
    return result   # sending result back 

#NOTE * check this functions has potential errors
